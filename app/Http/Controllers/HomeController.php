...
use DOMPDF;
use App\Models\activity_log_traders;
...
class HomeController extends Controller
{
    public function index(Request $request)
    {
        ...
        $viewPpk = DB::connection('sqlsrv2')->table('v_data_header')
            ...
            ->where( 'v_data_header.tgl_ppk', '>', Carbon::now()->subDays(5))
            ->select('ppks.*', 'v_data_header.*')
            ->orderBy('v_data_header.tgl_ppk', 'ASC')
            ->get();

        $viewRespon = DB::connection('sqlsrv2')->table('tr_respon_nsw')
            ->leftJoin("$dbView.ppks AS ppks", 'tr_respon_nsw.id_ppk', '=', "ppks.id_ppk")
            ->where('tr_respon_nsw.kd_kegiatan', 'E')
            ->select('ppks.*', 'tr_respon_nsw.*')
            ->get();      
    ...
    public function ajukanTanggal(Request $request, $id_ppk)
    {
        ...
        $mytime = Carbon::now();
        $email = session()->get('emailtrader');
        activity_log_traders::insert([
            'description' => 'traders mengajukan jadwal dengan id ppk : '.$id_ppk,
            'created_at' => $mytime,
            'ip' => $checkLocation->ip,
